...
@register_adv_est(AdvantageEstimator.PARTITION)  # or simply: @register_adv_est("partition")
...
@register_adv_est(AdvantageEstimator.PLOO)
...
def agg_loss(loss_mat: torch.Tensor, loss_mask: torch.Tensor, loss_agg_mode: str):
    ...
    if loss_agg_mode == "token-mean":
    ...
    elif loss_agg_mode == "seq-mean-token-mean-norm":
        token_counts = torch.sum(loss_mask, dim=-1)
        seq_losses = torch.sum(loss_mat * loss_mask, dim=-1)
        seq_mask = (token_counts > 0).float()
        token_counts = token_counts.clamp(min=1.0)
        seq_losses = torch.where(seq_mask > 0, seq_losses / token_counts, torch.zeros_like(seq_losses))
        seq_denominator = torch.sum(seq_mask).clamp(min=1.0)
        seq_mean = torch.sum(seq_losses * seq_mask) / seq_denominator  # seq-mean over non-empty sequences
...
def compute_policy_loss_wpmd(
    ...
    # Scale by max response length to normalize loss magnitude
    max_response_length = response_mask.shape[1]
    weighted_loss = torch.mean(seq_partition_weights * (seq_advantages / pmd_tau - torch.clamp(seq_log_prob - seq_old_log_prob, min=-20, max=20))**2)
...
def compute_policy_loss_cmdpo(
    ...
    # Get PMD-specific hyperparameters
    pmd_tau = config.policy_loss.get("pmd_tau", 0.01) if hasattr(config, "policy_loss") and config.policy_loss is not None else 0.01
    if extra_loss_kwargs is None or "uid" not in extra_loss_kwargs:
        raise ValueError("CMDPO loss requires 'uid' provided via extra_loss_kwargs.")
